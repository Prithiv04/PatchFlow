...
const DEFAULT_ROWS: ReportTableRow[] = [
...
export default function ReportPage() {
  ...
  const tableRows: ReportTableRow[] = DEFAULT_ROWS.map((r, i) => ({
    ...r,
    version: versionTag,
    ...
    >
      ...
      <motion.div variants={itemVariants} className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div className="space-y-2">
          ...
          <p className="text-muted text-sm">
            Full execution diff analysis for{" "}
        ...
        <div className="flex flex-wrap items-center gap-2.5 shrink-0">
          ...
          >
            ...
            <span>{jsonCopied ? "Downloaded!" : "Export Report JSON"}</span>
          </motion.button>
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="glass-card rounded-2xl p-5 border border-border flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-muted uppercase tracking-wider font-semibold">Match Confidence</p>
            <p className="text-2xl font-extrabold text-text">{confidencePercent}%</p>
          </div>
        </div>
        ...
        <div className="glass-card rounded-2xl p-5 border border-border flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-muted uppercase tracking-wider font-semibold">Render Hours Saved</p>
            <p className="text-2xl font-extrabold text-text">~2.5h</p>
          </div>
        </div>
        ...
        <div className="glass-card rounded-2xl p-5 border border-border flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-muted uppercase tracking-wider font-semibold">Version Tag</p>
            <p className="text-2xl font-extrabold text-primary font-mono">{versionTag}</p>
          </div>
        </div>
      </motion.div>

      {/* Patch Command Banner */}
      {(activePatch || patchReport) && (
        <motion.div
        ...
        >
          ...
          <div className="font-mono text-xs md:text-sm bg-black/40 border border-white/10 px-4 py-3 rounded-xl text-text/90 break-all">
            "{activePatch?.prompt || patchReport?.prompt || 'Replace target text across assets'}"
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4 pt-1 text-xs text-muted">
            <span className="flex items-center gap-1.5">
              ...
              Version: <span className="text-text font-bold font-mono ml-1">{versionTag}</span>
            </span>
            <span className="flex items-center gap-1.5">
              <Calendar className="w-3.5 h-3.5 text-primary" />
              Applied: <span className="text-text font-semibold ml-1">{activePatch?.created_at || patchReport?.applied_at || "Just now"}</span>
            </span>
          </div>
        </motion.div>
      )}
      ...
      <motion.div variants={itemVariants} className="glass-card rounded-2xl border border-border overflow-hidden shadow-xl">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {tableRows.map((row: ReportTableRow, idx: number) => (
                <tr
                  key={row.asset}
                ...
                >
                  ...
                  <td className="px-5 py-4 font-mono text-xs text-muted">{row.format}</td>
                  <td className="px-5 py-4 font-mono text-xs text-primary font-bold">{row.version}</td>
                  <td className="px-5 py-4">
                    <span className="px-2 py-0.5 rounded text-[11px] font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                      {row.status}
                    </span>
                  </td>
                  <td className="px-5 py-4 font-mono text-xs text-text">{row.confidence}%</td>
                  <td className="px-5 py-4 font-mono text-xs text-right font-bold text-text">{row.changes}</td>
                </tr>
              ))}
            </tbody>
          </table>
